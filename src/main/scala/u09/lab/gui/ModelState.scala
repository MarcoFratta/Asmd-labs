...
import scala.u09.lab.model.QMatrix.Pos2d

case class ModelState[S <: Pos2d[S]](step: Int, episode: Int, path: Seq[S]):
  def next[Q](hist: Seq[Seq[(S,Q)]]):ModelState[S] =
  ...
  def prev[Q](hist: Seq[Seq[(S,Q)]]):ModelState[S] =
  ...
  def getState[Q](hist: Seq[Seq[(S, Q)]]): (Int, Int) = (hist(episode)(step)._1.x, hist(episode)(step)._1.y)
  def getQ[Q](hist: Seq[Seq[(S, Q)]]): Q = hist(episode)(step)._2
