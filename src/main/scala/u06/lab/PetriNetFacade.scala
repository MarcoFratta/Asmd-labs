...
object PetriNetFacade:
  ...
  import Token.*
  import scala.u06.lab.PetriNetModel.Transition.*
  ...
  object PNOperation:
    ...
    private def arcOf[T: ClassTag](filter: Token[?] => Boolean)
    ...
    def moveToken(t:Token[?]): Arc = arcOf[Any](_ == t)(identity)(identity)(_ => List())
    ...
    def empty[T: ClassTag]: Arc = arcOf[T](_ => true)(_ => Token.Empty[T]())(identity)(_ => List())

    @targetName("Create an arc that moves a type T token")
    def <>[T: ClassTag]: Arc = arcOf[T](_ => true)(identity)(identity)(_ => List())
    @targetName("Create an arc that moves a a type T token token with the name s")
    def <>[T: ClassTag](s: String): Arc = arcOf[T](_.getName == s)(identity)(identity)(_ => List())
    @targetName("Moves a type T token after applying a function f to its value")
    def <>[T: ClassTag](f: T => ?): Arc = arcOf[T](_ => true)(f)(identity)(_ => List())
    @targetName("Moves a type T token with name s after applying a function f")
    def :~:[T: ClassTag](s: String)(f: T => ?): Arc = arcOf[T](_.getName == s)(f)(identity)(_ => List())
    @targetName("Moves a type T token mapping its name to s")
    def :~[T: ClassTag](s: String): Arc = arcOf[T](_ => true)(identity)(_ => s)(_ => List())
    @targetName("Move a type T token expanding it to a list of tokens using a function f")
    def ~+[T: ClassTag](f:T => Iterable[(?,String)]): Arc = arcOf[T](_ => true)(identity)(identity)(f)
    @targetName("Move a type T token with name s expanding it to a list of tokens using a function f")
    def :+[T: ClassTag](f: T => Iterable[(?, String)])(s:String): Arc = arcOf[T](_.getName == s)(identity)(identity)(f)

    @targetName("Move a type T token that satisfies the condition f")
    def ?>[T: ClassTag](f: T ?=> Boolean): Arc =
      arcOf[T](extract[T](_).exists(v => f(using v.getValue.get)))(identity)(identity)(_ => List())

    @targetName("Get the value of a type T token from the box m")
    def ><[T:ClassTag](using m: Box):T = >?[T](_ => true)
    @targetName("Get the value of a type T token, from the box m, whose name satisfies condition f")
    ...
    @targetName("Get the value of a type T token, from the box m, that has name n")
